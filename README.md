...
- **标签中文化**：构建期（`build_index.py`）调用 `scripts/genre_norm.py`，把每部作品 `tags`（多为日文/英文裸标签）翻译、归一化为中文 `tags_zh`，写入索引；站点优先展示中文标签。映射表来自多源 genre 数据（`data/genre/`，见 `SOURCES.md`），并以本仓库 `data/zh.json` 的 `tag_zh`（人工维护简中）优先覆盖。当前约 82.9% 的标签可映射为中文。
